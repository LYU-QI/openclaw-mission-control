...
async def _run_feishu_sync_flow(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        async with AsyncClient(
            ...
            history_resp = await client.get(f"/api/v1/feishu-sync/configs/{feishu_config_id}/history")
...
async def _run_mission_timeline_flow(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    try:
        async with AsyncClient(
            ...
            timeline_event_types = {item["event_type"] for item in timeline_resp.json()}
    ...
    try:
        async with AsyncClient(
            ...
            failed_subtask_resp = await client.patch(
                ...
                json={"status": "failed", "error_message": "approval path test", "result_risk": "high"},
            )
            assert failed_subtask_resp.status_code == 200
